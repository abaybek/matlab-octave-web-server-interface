classdef plotlyfig < handle
    ...
    methods
        ...
        function obj = plotlyfig(varargin)
            ...
            obj.UserData.Username = 'foo;';
            obj.UserData.ApiKey = 'bar';
            obj.UserData.PlotlyDomain = 'goo';
        ...
        function obj = loadplotlyref(obj)
            if isempty(obj.PlotlyReference)
                ...
                % plotly reference
                plotlyref = load(which('plotly_reference.mat'));
        ...
        function obj = update(obj)
            ...
            try
              ...
              ax = findobj(obj.State.Figure.Handle,'Type','axes');
              if( ~isempty(ax) )
                ix = strcmp(get(ax,'Tag'),'') | strcmp(get(ax,'Tag'),'PlotMatrixBigAx')| strcmp(get(ax,'Tag'),'PlotMatrixScatterAx');
                ax = ax(ix);
              end
            end
            ...
            for a = 1:length(ax)
                ...
                obj.State.Text(a).AssociatedAxis = plotly_handle(ax(axrev));
                obj.State.Text(a).Title = true;

                % find plots of figure
                plots = [ findobj(ax(axrev),'-not','Type','Text','-depth',1); ...
                          findobj(ax(axrev),'-not','Type','axes','-depth',1) ];
                ...
                for np = 1:length(plots)
                    ...
                    obj.State.Plot(obj.State.Figure.NumPlots).Handle = plotly_handle(plots(nprev));
                    obj.State.Plot(obj.State.Figure.NumPlots).AssociatedAxis = plotly_handle(ax(axrev));
                ...
                for t = 1:length(texts)
                    obj.State.Text(obj.State.Figure.NumTexts + t).Handle = plotly_handle(texts(t));
                    obj.State.Text(obj.State.Figure.NumTexts + t).Title = false;
                    obj.State.Text(obj.State.Figure.NumTexts + t).AssociatedAxis = plotly_handle(ax(axrev));
                end
            ...
            for g = 1:length(legs)
                obj.State.Legend(g).Handle = plotly_handle(legs(g));
            ...
            for c = 1:length(cols)
                ...
                obj.State.Colorbar(c).Handle = plotly_handle(cols(c));
